...
stack<string> fo;
void pos();
map<string,string> m; // map for storing filename and path of files moved to trash
...
void init_keyboard();  //for moving to non-canonical mode
void close_keyboard(); //for moving to canonical mode
int kbhit();  //for taking input from keyboard
int readch();
int coun=0;
int inde=0;
int y;
...
void  pos()  //saves the current active window size in struct windows
{
    int fp = 0;
...
void clearScreen()    //for clearing of the screen when new listing to be done
{
...
void goback(int x,int y)   //for moving the cursor to a particular location 
{
    printf("\033[%dA",x);
    printf("\033[%dD",y);
}
...
int lsfile(string c,int ind) //for listing of the files and directories present in the particular directory pointed by lsfile
{   
    coun=ind;
    struct dirent *de;  
...
void init_keyboard() //called when required to move to canonical mode. Stores the initial setting in initial setting
{
...
void close_keyboard()   //restores the initial settings
{
